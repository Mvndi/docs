...
- **Features**: The siege camp includes a banner with a fake beacon visible to players in the siege zone. Particles display a 4-block radius around the banner to indicate the capture area. Allies and members of the attacking nation can access the siege camp and teleport to it using command `/sc tp <town besieged>` (assuming it's not an active battle session), with a 10-second cooldown between teleports.
