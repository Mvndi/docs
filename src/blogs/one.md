# Mvndicraft IV dev log 1

Welcome back mvndibros it has indeed been a long while. We have been quietly working away towards finishing the server and we can confidently say that the release will be soon.

We made sure to overhaul and rust off older systems which hadn't been up to speed in the last iterations.

Today we would like to present our finished ships and the various mechanics that they will bring to the table since they are a game changer when it comes to sieges and trade.

Ships are a system which might as well be its own game at this point as their level of customisation is quite advanced we will be showing off their various features in this dev log.

A lot of what's talked about in this blog has more information which can be found on the [Boat](../guides/boats/boats.md) page

![1](https://files.catbox.moe/gcg5fd.png)

## New Seaman profession

The seaman will be the shipbuilding profession their task will mainly be to build and maintain ships while also being able to repair them at ports.

![2](https://files.catbox.moe/emhcco.png)

They bring new gadgets to the table like harpoons and greek fire grenades which were invented around the 7<sup>th</sup> century by the byzantines. They are a defensive impact grenade which will allow players to defend their walls and damage enemy vessels.

<video controls src="https://files.catbox.moe/4y9dgu.mp4" title="greek fire grenade"></video>

Ship upgrade list:

- Ram

- Storage upgrade

- Anchor winch

- Shield mounts upgrade

- Greek fire

- Trebuchet (Cog only)

- Ballista

- Cannon

(construction/upgrade times lowered for demonstration purposes, gui isn't massive the recorder just has scale 4)

<video controls src="https://files.catbox.moe/7uf3vf.mp4" title="shipbuilding"></video>

Ramming:

Ships have access to ramming without any upgrades but it causes quite a bit of damage to your own ship without the proper upgrades.

We have the drum upgrade which provides an ability (with cooldown) that temporarily increases the ship's speed proportionally to the amount of rowers you have.

The ram can also be installed at the front of a ship in order to cause even more damage to the ship being rammed and decrease the damage taken by the ramming ship.

<video controls src="https://files.catbox.moe/jt7f3x.mp4" title="ramming"></video>

Shield mounts upgrade:

Aside from being a lovely form of historically accurate decoration it will add a health bonus to the ship. Any type of mvndi shield can be installed on the ship.

![3](https://files.catbox.moe/90obmz.png)

Storage:

Ships have a certain amount of storage which you will be able to access when the ship is docked.

You will also be able to place horses inside the storage in order to launch amphibious assaults on enemy cities.

![4](https://files.catbox.moe/388x1t.png)

<video controls src="https://files.catbox.moe/zz9sss.mp4" title="horses"></video>


Full customisation

Ships will also be nameable and have a plethora of skins available to be unlocked through the achievement system.

![5](https://files.catbox.moe/4px5iw.png)

Next week we will post a dev log announcing the exact release date of the server and the Constantinople siege event, this dev log will elaborate on a raiding system which will heavily involve ship battles.

![6](https://files.catbox.moe/1e8ock.png)

We will have dev logs every week from now on up until the full release.

You have our deepest gratitude for sticking around with us we hope that you will enjoy this beautiful server which we've worked very hard on.
